...
fn url_to_addrs(url: &Url) -> Result<Vec<SocketAddr>> {
	...
	let mut addrs = (&host[..], port).to_socket_addrs()?.collect::<Vec<SocketAddr>>();
	addrs.dedup();
	Ok(addrs)
}
	...
	F: Factory,
{
	...
	pub fn connect(&mut self, poll: &mut Poll, url: Url) -> Result<()> {
		...
		let (tok, addresses) = {
			let (tok, entry, connection_id, handler) = if self.connections.len() < settings.max_connections {
									...
									if errno == CONNECTION_REFUSED {
										match self.connections[token.into()].reset() {
											Ok(_) => {
												...
												.or_else(|err| {
													self.connections[token.into()].error(Error::from(err));
													let handler = self.connections.remove(token.into()).consume();
													self.factory.connection_lost(handler);
													Ok::<(), Error>(())
												})
			...
			ALL => {
				...
				match cmd.into_signal() {
					...
					Signal::Timeout { delay, token: event } => {
						let timeout = self
							.timer
							.set_timeout(Duration::from_millis(delay), Timeout { connection: ALL, event });
						for (_, conn) in self.connections.iter_mut() {
